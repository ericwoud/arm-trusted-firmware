...
static struct uuid_to_filename_table {
	char * name;
	uuid_t uuid;
} uuid_to_filename[] = {
	{
			 ...
			 io_entity_t *entity)
{
	...
	static const uuid_t uuid_dtb = UUID_NT_FW_CONFIG;
	static const uuid_t uuid_initrd = UUID_SECURE_PAYLOAD_BL32_EXTRA2;
	size_t bytes_read;
		...
		if (result == 0) {
			...
			do {
				if (compare_uuids(&uuid_to_filename[i].uuid, 
						  &uuid_dtb) == 0)
					uuid_to_filename[i].name = NULL;
				if (compare_uuids(&uuid_to_filename[i].uuid, 
						  &uuid_initrd) == 0)
					uuid_to_filename[i].name = NULL;
			} while (compare_uuids(&uuid_to_filename[++i].uuid,
					       &uuid_null) != 0);
			goto uboot_skip;
		}
	}
	...
	if (!uuid_to_filename[i].name) {
		INFO("fat_file_open: loaded /boot/u-boot.bin, skipping initrd/dtb\n");
		goto fat_file_open_failed;
	}
